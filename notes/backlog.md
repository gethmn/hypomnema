...
- **Static sqlite-vec bundling.** Move sqlite-vec from an operator-provisioned dynamic-library prerequisite to a statically linked extension registered by `hmnd`, so `cargo install hypomnema` produces runnable binaries without a separate sqlite-vec download. Do sooner rather than later; this removes a recurring install/dev-shell/CI footgun and aligns with the self-contained Rust deployment rationale. Source proposal: [`notes/proposals/static-sqlite-vec-bundling.md`](proposals/static-sqlite-vec-bundling.md); delivery stories: [`notes/proposals/static-sqlite-vec-bundling-stories.md`](proposals/static-sqlite-vec-bundling-stories.md).
