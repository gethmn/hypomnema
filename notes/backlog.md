...
- ~~**Static sqlite-vec bundling.**~~ **Pulled into round 11** (step 22). sqlite-vec is now statically linked via the `sqlite-vec` Rust crate and registered with `sqlite3_auto_extension` at `hmnd` startup; `extension_path` and `HYPOMNEMA_VEC_EXT_PATH` are gone. Source proposal: [`notes/proposals/static-sqlite-vec-bundling.md`](proposals/static-sqlite-vec-bundling.md).
...
- ~~**`flake.nix` sqlite-vec dylib provisioning.**~~ **Resolved by round 11 / step 22 static bundling.** sqlite-vec is statically linked into `hmnd` via the `sqlite-vec` Rust crate, so the dev shell no longer needs to provision a dylib.
...
## Product-level boundaries — pointer only

The canonical current-state matrix lives in [`docs/product/vision.md` § Current Product Boundaries](../docs/product/vision.md#current-product-boundaries): writes-to-vault, the ownership model, bridge-managed-files format spec, conflict resolution, durable/replayable events, multi-instance coordination, Obsidian-specific behavior, bidirectional sync, and metadata features such as structured frontmatter/tags/backlinks.

Items in the vision boundary table are not automatically queued for a future round. If one moves into active scope, it becomes a backlog item here and then a round entry; until then, vision.md is the home.
