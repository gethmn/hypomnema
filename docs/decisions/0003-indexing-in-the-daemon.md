...
This amendment changes no behavior; it clarifies the boundary so the project's positioning ("a directory of notes" rather than "Markdown only", per `docs/product/vision.md` § Current Product Boundaries → "Semantic indexing beyond Markdown") doesn't silently contradict the ADR.
