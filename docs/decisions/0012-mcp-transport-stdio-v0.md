...
- **Round-2 kept `hmnd` off the `rmcp` dependency path.** The daemon's dependency graph stayed narrower than it would have if step 8 had landed MCP on `hmnd` directly. `hmn` grew by `rmcp` (with `["server", "transport-io", "macros", "schemars"]`); `hmnd` was unchanged on the MCP axis until HTTP-MCP shipped later in ADR-0013. (Net new dependencies for v0 are recorded in [step-08 workplan § Net new dependencies](../roadmap/step-08-workplan.md#net-new-dependencies).)
...
- **Round-2 shipped no MCP write tools, prompts, resources, or sampling.** Step 8 shipped read-only search tools only. Round 3+ pulled up vault-management MCP tools per [ADR-0011](./0011-vault-management-on-hmn.md); other rmcp 1.5.0 protocol features (prompts, resource subscriptions, sampling) remain future concerns unless specifically amended.
...
stdio-shipped / socket-deferred decision recorded in this ADR is
