...
| `max_retries` | integer | no | `1` | Maximum retries on transport-level failures or HTTP `5xx`. Backoff before retry is 250ms. `4xx` responses are not retried by this mechanism (they signal a bug, not a transient condition). Note this is distinct from adaptive batch-size shrinking, which *does* reissue a request after a `400`/`413`/`422` on an over-large batch — see `batch_size` — so operators may see a few `4xx` during batch-size probing independent of `max_retries`. Set to `0` to disable retries. |
