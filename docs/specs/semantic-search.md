...
| `limit` | integer | no | 10 | Global result cap after cross-vault merge. In `document` mode this caps returned documents; in `chunk` mode this caps returned chunks. Validation: `1..=1000`. |
...
- `limit`: must be in `1..=1000` when supplied. Out-of-range values return `invalid_request`.
...
Evidence chunks deliberately **omit** `path` and `content_hash`. The parent document result carries the canonical source reference, and because documents are grouped by `(vault, path, content_hash)`, every evidence chunk shares the parent's exact `path` and `content_hash` — repeating them per chunk would be pure duplication. Consumers that need the source reference for an evidence chunk read it from the enclosing document. This is an intentional contract (covered by serde and live-daemon tests), not accidental drift; flat `chunk`-mode results carry both fields because they have no parent.
