...
| Inbound | MCP over stdio (via `hmn mcp`) | Agent consumers that spawn subprocesses (Claude Code, Iris) — search + vault-management request/response tools; live `vault_watch` deferred pending rmcp verification |
