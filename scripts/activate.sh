#!/bin/sh
# Full environment activation for Duo
...
if [ -z "${IN_NIX_SHELL:-}" ] && [ -f "$HYPOMNEMA_ROOT/flake.nix" ]; then
    if check_tool nix; then
        ...
        eval "$(nix print-dev-env "$HYPOMNEMA_ROOT" 2>/dev/null)" || {
...
if [ -f "$HYPOMNEMA_ROOT/pyproject.toml" ]; then
...
if [ -t 1 ] && [ -z "${HYPOMNEMA_BANNER_SHOWN:-}" ]; then
    HYPOMNEMA_BANNER_SHOWN=1
    export HYPOMNEMA_BANNER_SHOWN
